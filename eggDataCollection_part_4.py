#Clearing raw text files then making it CLEAN and CSV by meta's Llama-4-Scout-17B-16E-Instruct model
...
def strLOFDToDf(resp):
    ...
    import json
    import re
    # Remove backticks if AI wraps response
    cleaned = re.sub(r'^```(?:json)?\s*|\s*```$', '', resp.strip())
    list_of_dicts = json.loads(cleaned)
    return pd.DataFrame(list_of_dicts)


import re

def collectEggDataMeta(source='swapno'):
    ...
    if not files:
        print(f'No raw text files found for {src}_{product}')
        return
    ...
    prompt1 = """Extract all chicken egg items from the grocery page and output them in JSON format with the following structure:
IMPORTANT: Output ONLY the JSON list of dictionaries. Do not include any explanations, notes, additional text, or formatting. The response must be a valid JSON array starting with [ and ending with ].
[
    {
        "product_name": "name of the egg product (only chicken egg)",
        "category": "loose or packet",
        "price": "price in numbers only (without ৳ symbol)",
        "number_of_unit": "number of eggs just the number (e.g., 4, 12, etc.)",
...
from the given text: """ + raw_text
    ...
    resp, model_used = respGenerator(role=role, prompt=prompt1)
    ...
    prompt2 = """Extract the brand name from each product title and return a JSON array of {"brand_name"}. otherwise use 'loose'. IMPORTANT: Output ONLY the JSON list of dictionaries. Do not include any explanations, notes, additional text, or formatting. The response must be a valid JSON array starting with [ and ending with ].
[the output is only list of dictionaries without any extra text]
the product titles are: """ + ', '.join(df['product_name'].tolist())
...
def collectEggDataFromAllSourcesMeta():
    collectEggDataMeta(source='chaldal')
    collectEggDataMeta(source='swapno')